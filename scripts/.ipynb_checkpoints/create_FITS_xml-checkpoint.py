...
# etree.register_namespace("pds", "https://pds.nasa.gov/pds4/pds/v1/")
...
for PJ in range(13, 37):
    ...
    modification_history[0].xpath("//*[local-name() = 'modification_date']")[0].text = '2025-07-29'
    ...
    time_coordinates = observation_area.xpath("//*[local-name() = 'Time_Coordinates']")[0]

    identification_area.xpath("//*[local-name() = 'title']")[0].text = f"PJ {PJ} mosaic"

    time_coordinates.xpath("//*[local-name() = 'start_date_time']")[0].text = PJ_times[f"{PJ}"]["start"]
    time_coordinates.xpath("//*[local-name() = 'stop_date_time']")[0].text = PJ_times[str(PJ)]["end"]
    ...
    et.write(f"data_mosaics/PJ{PJ}/junocam_calibration_PJ{PJ}.xml", xml_declaration=True, encoding='UTF-8')
